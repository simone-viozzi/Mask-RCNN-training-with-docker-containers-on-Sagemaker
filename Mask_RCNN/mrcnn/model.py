...
def load_image_gt_multiproc(dataset, config, image_ids, augmentation=None, use_mini_mask=False):
    """Load and return ground truth data for an image (image, mask, bounding boxes).
    ...
    for i, image_id in enumerate(image_ids):
        ...
        original_shapes.append(images[i].shape)

        images[i], window, scale, padding, crop = utils.resize_image(
            images[i],
            min_dim=config.IMAGE_MIN_DIM,
            min_scale=config.IMAGE_MIN_SCALE,
            max_dim=config.IMAGE_MAX_DIM,
        ...
        masks[i] = utils.resize_mask(masks[i], scale, padding, crop)
    ...
    for i, image_id in enumerate(image_ids):
        ...
        masks[i] = masks[i][:, :, _idx]
        classes_ids[i] = classes_ids[i][_idx]
        
        # Bounding boxes. Note that some boxes might be all zeros
        # if the corresponding mask got cropped out.
        # bbox: [num_instances, (y1, x1, y2, x2)]
        bboxs.append(utils.extract_bboxes(masks[i]))
...
def data_generator_multiproc(dataset, config, shuffle=True, augment=False, augmentation=None,
                   random_rois=0, batch_size=1, detection_targets=False,
                   no_augmentation_sources=None):
    """A generator that returns images and corresponding target class ids,
    bounding box deltas, and masks.
...
class MaskRCNN():
    ...
    def find_last(self):
        ...
        if not dir_names:
            ...
            raise FileNotFoundError(
                errno.ENOENT,
                f"Could not find model directory under {self.checkpoints_dir}")
        # Pick last directory
        dir_name = os.path.join(self.checkpoints_dir, dir_names[-1])
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        if not os.path.exists(self.checkpoints_dir_unique):
            os.makedirs(self.checkpoints_dir_unique)
        if not os.path.exists(self.tensorboard_dir_unique):
        ...
        # Nota: histogram_freq=1 genera un errore.. 
        # 
        callbacks = [
